...
def installLocal(lib):
    subprocess.check_call([
        sys.executable,
        "-Wignore","-u","-m","pip","--disable-pip-version-check","install","--no-clean","--target",f"{os.getcwd()}",f"{lib}"
    ]);
def conda(lib):
    try:
        subprocess.check_call([
            sys.executable,
            "-Wignore","-u","-m","conda","install","--yes",f"{lib}"
        ]);
    except Exception as e:
        print(f"{e}")
...
def importLibAs(lib, alias):
  ...
  if re.search(r"[^a-zA-Z0-9_\]", alias):
    print("Invalid characters in library alias.")
    return None
  try:
    module = __import__(lib)
...
def importModuleAs(lib, pkg, alias):
  ...
  if re.search(r"[^a-zA-Z0-9_\.]", pkg):
    ...
    return None
  if re.search(r"[^a-zA-Z0-9_\]", alias):
    print("Invalid characters in module alias.")
    return None
  try:
    globals()[lib] = __import__(lib, globals(), locals(), [pkg], 0)
