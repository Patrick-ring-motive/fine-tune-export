...
function createOutputBuffer(_ref) {
  ...
  function insertBeforeLastWhitespace(textToInsert) {
    ...
    if (!(0, _stringUtils.isWhitespace)(buffer.charCodeAt(bufferIndex - 1))) {
      // no trailing whitespaces
      push(textToInsert);
      return;
    }
    while ((0, _stringUtils.isWhitespace)(buffer.charCodeAt(bufferIndex - 1))) {
      bufferIndex--;
    }
    if (bufferIndex <= 0) {
      throw new Error("Cannot insert: ".concat(flushedMessage));
  ...
  function endsWithIgnoringWhitespace(char) {
    ...
    while (i > 0) {
      ...
      if (!(0, _stringUtils.isWhitespace)(buffer.charCodeAt(i))) {
        return false;
      }
      i--;
    }
